...
char * BubbleSort(char B[]);
void SwapInt(int A[], int i);
void SwapChar(char B[], int i);
...
void SwapInt(int A[], int i) {
    ...
    for(int j = 0; j < 9; j++) {
        if (j <= 7) { printf("%d, ", A[j]); }
        else printf("%d] Cursor Position: %d", A[j], A[i]);
    }
}  
...
char * BubbleSort (char B[]) {
    ...
    void SwapChar(char B[], int i) {
        ...
        printf("\n[ ");
        for(int j = 0; j < 12; j++) {
            if (j <= 10) { printf("%c ", B[j]); }
            else printf("[%c] Cursor Position: %c", B[j], B[i]);
        }
    }
    /*
    Question 3:
        ...
        to find big-O we must ignore constants, so ( (n/2)^3 ) becomes ( n^3 ) and +2 can be removed entirely 
        therefore, the big-O of this problem is O(n^3)
